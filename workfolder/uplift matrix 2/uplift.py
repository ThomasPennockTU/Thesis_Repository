...
# === CONFIGURATION ===
odb_path = r'D:\tpennock\GitHub\Thesis_Repository\1.0-First delta flume final matrix 2\Job-H-0_8_2300_30.odb'
output_csv = 'relative_uplift_timeseries_08_ASC.csv'
...
# === DEFINE HALF BLOCKS ===
half_block_names = set(['B-%d-1' % i for i in range(1, 22, 2)] + ['B-%d-10' % i for i in range(1, 22, 2)])
...
# === EXTRACT RELATIVE UPLIFT PER TIME FRAME ===
step = odb.steps.values()[1]
n_total_frames = len(step.frames)
frame_data = []

print("Number of frames detected:", n_total_frames)
...
for i in range(n_total_frames):
    ...
    frame_data.append([time] + uplift_values)
    print("Processed frame %d / %d (Time = %.4f)" % (i + 1, n_total_frames, time))

# === EXPORT TO CSV (PYTHON 2.7 COMPATIBLE) ===
with open(output_csv, 'wb') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(['Time'] + sorted_blocks)

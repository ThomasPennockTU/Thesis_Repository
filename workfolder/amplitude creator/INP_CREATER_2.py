...
# Parameters
Hs_values = [1.0, 1.2, 1.4, 1.6, 1.8]  # Significant wave heights [m]
N = 1000                              # Total number of waves
n_peaks = 5                           # Top 5 waves: 1 in 1000 to 5 in 1000
...
for Hs in Hs_values:
    sigma = Hs / np.sqrt(2)
    exceed_probs = np.arange(n_peaks, 0, -1) / N
    H_extreme = sigma * np.sqrt(-2 * np.log(exceed_probs))
    results.append([Hs] + list(H_extreme[::-1]))  # reverse to get 1/1000 first

# Create DataFrame
columns = ['Hs [m]'] + [f'{i} in 1000 wave [m]' for i in range(1, n_peaks + 1)]

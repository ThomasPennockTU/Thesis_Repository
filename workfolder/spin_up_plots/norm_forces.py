...
node_label = 33
node_set_name = 'whole'  # Changed from 'sides' to 'whole'
...
# --- Function to find CNORMF key ---
def get_cnormf_key(step):
    for key in step.frames[0].fieldOutputs.keys():
        if key.startswith('CNORMF'):
            return key
    raise ValueError("CNORMF field not found in step '{}'.".format(step.name))
...
for step_name in odb.steps.keys():
    ...
    cnormf_key = get_cnormf_key(step)
    process_step(step, cnormf_key, t_offset=cumulative_time)
    if step.frames:
...
plt.savefig("cnormf_slope_components_allsteps_node{}_{}_whole_1000.png".format(

...
# Abaqus input file names
template_inp_filename = "Job-200.inp"
...
for i, H_wave in enumerate(wave_heights):
    ...
    wave_name = f"Wave {i+1} in 1000, H={H_wave:.3f}"
...
print(f"\n✅ Finished computing {n_waves} waves.")

# ------------------------------
# PLOT RESULTS
# ------------------------------

# Plot only the top 5 waves to keep plots readable
top_waves = list(wave_pressures.keys())[:5]
...
for idx, wave_name in enumerate(top_waves):
...
plt.title("Wave Pressure, Filter Response, and Residuals (Top 5 Waves)")
plt.legend(ncol=2, fontsize=8)
plt.grid(True)
plt.tight_layout()
plt.show()

# Plot bar charts for the top 5 waves
fig, axs = plt.subplots(len(top_waves), 1, figsize=(10, 3*len(top_waves)), sharex=True)

for i, wave_name in enumerate(top_waves):
    axs[i].bar(np.arange(1, 23), block_pressures_per_wave[wave_name],
               color='skyblue', width=1, edgecolor='black')
...
inp_path = os.path.join(script_dir, template_inp_filename)
output_path = os.path.join(script_dir, f"{job_name}_ASC.inp")
